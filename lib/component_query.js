    ...
    return function(err,client,done){
        ...
        result.on('end',function(){
            if(!error_condition){
                ...
                task.components=fix_component_names(direction,Object.keys(components))
                next(null,task)
            }
            done()
            return null
...
function fix_component_names(direction,components){
    var patterns= {'wim':/wimid_(\d*)/
                  ,'vds':/(\d{5,7})/
                  }
    console.log(direction)
    var dir = direction.substr(0,1).toUpperCase()
